classdef predict_emotions_from_reasons < load_data.load_data
%example: obj = stats.predict_emotions_from_reasons('responses_pilot/Music Listening Habits.csv','AllResponses'); do_predict_emotions_from_reasons(obj);

    properties
    end
    methods
        function obj = predict_emotions_from_reasons(dataPath,filterMethod)
            if nargin < 2
        ...
        function obj = do_predict_emotions_from_reasons(obj)
            close all
            ...
            for k = 1:numel(reasonTypes)
                ...
                for j = 1:numel(reasonLabels)
                    FactorNames = {'TendernessLove','StrengthEnergy','PainSadness','AmusementHappiness','PleasureBeauty',reasonLabels{j}};
